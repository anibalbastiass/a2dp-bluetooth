...
import a2dp.vol.widget.ConnectWidget;
import a2dp.vol.db.DeviceDB;
import a2dp.vol.activity.MainActivity;
import a2dp.vol.R;
...
public class AudioService extends Service implements OnAudioFocusChangeListener {
    ...
    public static BtDeviceModel[] btdConn = new BtDeviceModel[20]; // n the devices in the
    // database that has
    // connected
    ...
    private static A2DPApplication application;
...
*/
    ...
    public void onDestroy() {
        ...
        stopService(new Intent(application, StoreLocationService.class));
        // close the database
        try {
    ...
    protected void DoConnected(BtDeviceModel bt2) {
        boolean done = false;
    ...
    protected void DoDisconnected(BtDeviceModel bt2) {
        ...
        if (bt2 != null && bt2.isGetLoc() && !isMyServiceRunning(StoreLocationService.class)) {
            Intent dolock = new Intent(AudioService.this, StoreLocationService.class);
            ...
            for (int id : ids) {
                ...
                if (bt_mac != null)
                    if (bt_mac.length() == 17) {
                        ...
                        if (btdConn != null)
                            for (BtDeviceModel device : btdConn) {
                                String mac = "";
    ...
    private boolean runApp(BtDeviceModel bt) {
            ...
            switch (MessageStream) {
                case IN_CALL_STREAM:
                    ...
                    if (Build.VERSION.SDK_INT < android.os.Build.VERSION_CODES.O) {
                        am2.requestAudioFocus(AudioService.this,
            ...
            if (A2DP_Vol.equalsIgnoreCase(uttId)) {
                ...
                switch (MessageStream) {
                    case IN_CALL_STREAM:
                        ...
                        result = am2.abandonAudioFocus(AudioService.this);
                        if (musicWasPlaying) {
    ...
    public BroadcastReceiver sco_change = new BroadcastReceiver() {
        ...
        public void onReceive(Context arg0, Intent arg1) {
            ...
            if (state == AudioManager.SCO_AUDIO_STATE_CONNECTED && Build.VERSION.SDK_INT >= android.os.Build.VERSION_CODES.O && arg0 == AudioService.this) {
                if (afr == null) {
                    AudioAttributes aa = new AudioAttributes.Builder()
                            .setUsage(AudioAttributes.USAGE_VOICE_COMMUNICATION).setLegacyStreamType(AudioManager.STREAM_VOICE_CALL)
                            .setContentType(AudioAttributes.CONTENT_TYPE_SPEECH).setFlags(AudioAttributes.FLAG_AUDIBILITY_ENFORCED).build();
            ...
            if (state == AudioManager.SCO_AUDIO_STATE_DISCONNECTED && !clearedTts && arg0 == AudioService.this) {
                if (!mTtsReady)
                    mTts = new TextToSpeech(application, listenerStarted);
                ...
                am2.requestAudioFocus(AudioService.this, AudioManager.STREAM_MUSIC,
                        AudioManager.AUDIOFOCUS_GAIN_TRANSIENT);
                ...
                if (Build.VERSION.SDK_INT < android.os.Build.VERSION_CODES.O) {
                    am2.abandonAudioFocus(AudioService.this);
                } else {
